...
export default class Index extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return (
      nextProps.pathname !== this.props.pathname || nextState !== this.state
    ); //route 登录和注册共享一份组件强制更新
  }
  ...
  test(key, value) {
    switch (key) {
      ...
      case "passwords":
        return (
          this.state.password === value && /^[a-zA-Z0-9]{6,15}$/.test(value)
        );
    }
  }
  ...
  change(e) {
    ...
    this.setState({ [name]: e.target.value }, () => {
      this.verifyAndError(this.state);
    });
  }
  ...
  verifyAndError({ error, captcha, ...filds }) {
