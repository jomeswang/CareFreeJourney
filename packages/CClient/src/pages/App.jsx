...
class App extends Component {
  ...
  componentDidMount() {
     var wow = new WOW({
       ...
       offset: 10, // 距离元素多少距离执行 (默认为 0)
       mobile: false, // 是否在移动设备执行动画 (默认为 true)
       live: false, // 是否异步加载内容 (默认为 true)
     });
     wow.init();
     console.log('wow', wow);
  }
  render() {
    if (this.props.Appstore.app.error !== null)
      return (
        <Redirect
          to={{
            pathname: "/404",
            state: { error: this.props.Appstore.app.error },
          }}
        />
      );
    return (
